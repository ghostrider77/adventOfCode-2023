...
let distance ({x = x1; y = y1} : coord) ({x = x2; y = y2} : coord) ({rows; columns} : expansion) : int =
  ...
  let row_correction = List.fold_left (fun acc ix -> if down < ix && ix < up then acc + 1 else acc) 0 rows in
  let column_correction = List.fold_left (fun acc jy -> if left < jy && jy < right then acc + 1 else acc) 0 columns in
